...
const mentorNick = m => nickOf(m.id || m.name, m.nickname);
/* 경력 타임라인의 세부내용.
...
function avatarHtml(m, cls){
  ...
  /* 아바타 이니셜도 실명이 아니라 닉네임 첫 글자를 쓴다 — 카드·상세와 같은 이름이어야 한다. */
  return `<div class="avatar ${cls}" style="${avatarStyle(palOf(m.id))}">${escapeHTML(mentorNick(m).charAt(0))}</div>`;
}
...
function getFilteredMentors(){
  ...
  let list = MENTORS.filter(m=>{
    ...
    if (searchQuery){
      /* 닉네임으로도 검색되게 mentorNick 을 넣는다. 실명(m.name)은 내부 데이터라
         화면엔 안 나가지만, 검색어로는 받아 준다 — 본인이 자기 실명으로 찾을 수 있다. */
      const hay = [mentorNick(m), m.name, m.company, m.role, categoryName(m.jobMajor),
...
function renderSearch(){
  ...
  grid.innerHTML = pageItems.map(m=>`
    <div class="mentor-card" onclick="openProfile('${escapeHTML(m.id)}')">
      ...
      <div class="mc-body">
        <div class="mc-info">
          <div class="mc-name">${mentorNick(m)}</div>
      ...
      <div class="ph-top">
        ...
        <div class="ph-id">
          <div class="ph-name-row">
            <span class="ph-name">${mentorNick(m)}</span>
