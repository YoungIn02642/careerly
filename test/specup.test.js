...
/* 파라미터 이름을 못 박는다. plcyKywdNm 은 정해진 어휘 17종(교육지원·보조금·인턴…)의
   필드라 '공모전' 을 넣으면 0건이고, 이름이 틀린 파라미터는 **무시되고 전체 2,715건이
   그대로 온다.** 둘 다 에러가 아니라서 화면만 봐서는 못 잡는다. */
ok('정책 이름 검색 파라미터를 쓴다',
   require('fs').readFileSync(require('path').join(__dirname, '..', 'backend', 'src', 'specup.js'), 'utf8')
     .includes('&plcyNm=${encodeURIComponent(kw)}'),
   'plcyKywdNm 으로 부르면 전부 0건이다 (실측)');
