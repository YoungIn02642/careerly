...
/* 한 글자 기호 ㈜ 와 전각 괄호 （주）. 고용24 캐시 3,851건 중 412건(11%)이 이 표기라
   빠뜨리면 그 회사들이 **에러 없이** 조회에서 사라진다 (2026-09-06 실측으로 발견). */
ok('한 글자 ㈜ 를 뗀다', NAME.sameCompany('㈜면사랑', '면사랑'));
ok('뒤에 붙은 ㈜ 도 뗀다', NAME.sameCompany('넥스큐브코퍼레이션㈜', '넥스큐브코퍼레이션'));
ok('전각 괄호 （주） 도 뗀다', NAME.sameCompany('（주）토스', '토스'));
/* 한글로 적은 영문 사명. 고용24 에는 `엘지전자` 로 올라와 있는데 우리 목록(DART)은
   `LG전자` 라, 이게 없으면 그 회사 공고가 통째로 안 잡힌다 (2026-09-07 실측). */
ok('엘지 = LG', NAME.sameCompany('엘지전자', 'LG전자'));
ok('에스케이 = SK', NAME.sameCompany('에스케이하이닉스', 'SK하이닉스'));
ok('씨제이 = CJ', NAME.sameCompany('씨제이제일제당', 'CJ제일제당'));
/* 다른 낱말 안에 우연히 든 '엘지' 가 바뀌어도, 대조가 **정확히 같을 때만** 통과라
   오탐이 되지 않는다. 이 검사가 그 안전장치를 지킨다. */
ok('  낱말 안의 우연한 일치는 붙지 않는다', !NAME.sameCompany('임마누엘지역아동센터', 'LG전자'));
