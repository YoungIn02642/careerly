...
console.log('\n── 3. 이름·회사·학교는 넘긴 목록으로 가린다 ──');
ok('이름', anon('저는 홍길동입니다', { names: ['홍길동'] }) === '저는 [이름]입니다');
ok('회사', anon('삼성전자 인턴을 했습니다', { orgs: ['삼성전자'] }) === '[회사·학교] 인턴을 했습니다');
ok('학교', anon('한국대학교 컴퓨터공학과', { orgs: ['한국대학교'] }) === '[회사·학교] 컴퓨터공학과');
ok('긴 것 먼저(부분겹침)', anon('서울대와 서울대학교', { orgs: ['서울대', '서울대학교'] }) === '[회사·학교]와 [회사·학교]');
ok('한 글자 목록은 무시', anon('김 대리와 회의', { names: ['김'] }) === '김 대리와 회의');

console.log('\n── 4. masked 요약을 정확히 센다 ──');
const r = A.anonymize('저는 홍길동, 010-1234-5678, a@b.com 입니다', { names: ['홍길동'] });
const byType = Object.fromEntries(r.masked.map(m => [m.type, m.count]));
ok('이름 1', byType['이름'] === 1, JSON.stringify(r.masked));
